...
class OrdersController {
  ...
  private registerRoutes(): void {
    this.router.get("/orders/:_id([a-f0-9A-F]{24})", this.getOrders);
    this.router.get("/order/:_id([a-f0-9A-F]{24})", this.getOrder);
  ...
  private async getOrders(
    ...
    try {
      ...
      const orders = await ordersService.getOrders(_id);
